...
_User should first allow the bridge to transfer tokens on his behalf.
  ...
  Note that Lineth can reserve some tokens (which use a dedicated bridge).
  In this case, the token cannot be bridged. Lineth can only reserve tokens
  that have not been bridged yet.
  Lineth can pause the bridge for security reason. In this case new bridge
...
_Deploy a new EC20 contract for bridged token using a beacon proxy pattern.
  To adapt to future requirements, Lineth can update the implementation of
...
_Lineth can reserve tokens. In this case, the token cannot be bridged.
  Lineth can only reserve tokens that have not been bridged before.
...
_Lineth can set a custom ERC-20 contract for specific ERC-20.
  For security purposes, Lineth can only call this function if the token has
