...
Initialization data structure for the LinethRollup contract.
...
struct FinalizationDataV3 {
  ...
  struct ILinethRollup.ShnarfData shnarfData;
...
Emitted when the LinethRollup contract version has changed.
...
```solidity
function submitBlobs(struct ILinethRollup.BlobSubmission[] _blobSubmissions, bytes32 _parentShnarf, bytes32 _finalBlobShnarf) external
```
...
```solidity
function submitDataAsCalldata(struct ILinethRollup.CompressedCalldataSubmission _submission, bytes32 _parentShnarf, bytes32 _expectedShnarf) external
```
...
```solidity
function finalizeBlocks(bytes _aggregatedProof, uint256 _proofType, struct ILinethRollup.FinalizationDataV3 _finalizationData) external
```
