...
```solidity
function __LinethRollup_init(struct ILinethRollup.InitializationData _initializationData) internal virtual
```
...
```solidity
function submitBlobs(struct ILinethRollup.BlobSubmission[] _blobSubmissions, bytes32 _parentShnarf, bytes32 _finalBlobShnarf) external virtual
```
...
| Name | Type | Description |
| ---- | ---- | ----------- |
| _blobSubmissions | struct ILinethRollup.BlobSubmission[] | The data for blob submission including proofs and required polynomials. |
...
```solidity
function _submitBlobs(struct ILinethRollup.BlobSubmission[] _blobSubmissions, bytes32 _parentShnarf, bytes32 _finalBlobShnarf) internal virtual
```
...
```solidity
function submitDataAsCalldata(struct ILinethRollup.CompressedCalldataSubmission _submission, bytes32 _parentShnarf, bytes32 _expectedShnarf) external virtual
```
...
| Name | Type | Description |
| ---- | ---- | ----------- |
| _submission | struct ILinethRollup.CompressedCalldataSubmission | The supporting data for compressed data submission including compressed data. |
| _parentShnarf | bytes32 | The parent shnarf used in continuity checks as it includes the parentStateRootHash in its computation. |
...
```solidity
function _submitDataAsCalldata(struct ILinethRollup.CompressedCalldataSubmission _submission, bytes32 _parentShnarf, bytes32 _expectedShnarf) internal virtual
```
...
```solidity
function finalizeBlocks(bytes _aggregatedProof, uint256 _proofType, struct ILinethRollup.FinalizationDataV3 _finalizationData) external virtual
```
...
| _finalizationData | struct ILinethRollup.FinalizationDataV3 | The full finalization data. |

### _finalizeBlocks

```solidity
function _finalizeBlocks(struct ILinethRollup.FinalizationDataV3 _finalizationData, uint256 _lastFinalizedBlock) internal virtual returns (bytes32 finalShnarf)
```
