...
2. The `L1 Token` is either:
   ...
   - Burned
3. `L1 Token Bridge` sends a message using `sendMessage()` to `LinethRollup / L1MessageService`.
4. The contract:
...
12. **LinethRollup / L1MessageService** verifies rolling hash sync as part of a feedback loop.
