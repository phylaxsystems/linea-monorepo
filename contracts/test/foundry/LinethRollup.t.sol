...
import { ILinethRollup } from "src/rollup/interfaces/ILinethRollup.sol";
...
contract LinethRollupTestHelper is LinethRollup {
...
contract LinethRollupTest is Test {
  ...
  function setUp() public {
    ...
    ILinethRollup.InitializationData memory initData;
    ...
    bytes memory initializer = abi.encodeWithSelector(LinethRollup.initialize.selector, initData);
  ...
  function testSubmitDataAsCalldata() public {
    ILinethRollup.CompressedCalldataSubmission memory submission;
    ...
    bytes32 dataEvaluationClaim = linethRollup.calculateY(submission.compressedData, dataEvaluationPoint);

    bytes32 parentShnarf = linethRollup.computeShnarf(0x0, 0x0, 0x0, 0x0, 0x0);
    ...
    vm.prank(operator);
    linethRollup.submitDataAsCalldata(submission, parentShnarf, expectedShnarf);

    uint256 exists = linethRollup.blobShnarfExists(expectedShnarf);
