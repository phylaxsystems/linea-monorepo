...
LINETH_ROLLUP_ADDR=$(sed -nE 's/.*"LinethRollupV8":[[:space:]]*"(0x[a-fA-F0-9]{40})".*/\1/p' "$PRECOMPUTED" | head -1)
...
echo "$LINETH_ROLLUP_ADDR" | grep -qE '^0x[a-fA-F0-9]{40}$' || die "LinethRollupV8 missing from $PRECOMPUTED"
...
log "LinethRollupV8 = $LINETH_ROLLUP_ADDR"
...
export L1_RPC_URL L2_CHAIN_ID LINETH_ROLLUP_ADDR L2_MS_ADDR
