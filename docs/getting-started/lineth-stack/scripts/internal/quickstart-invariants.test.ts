...
void (async () => {
  await test("computes genesis shnarf with the LinethRollup V8 helper shape", () => {
    assert.equal(
  ...
  await test("precomputes boot-critical addresses from deterministic deployer nonces", () => {
    assert.deepEqual(
      ...
      {
        l1LinethRollup: "0xDc64a140Aa3E981100a9becA4E685f962f0cF6C9",
        l2MessageService: "0x948B3c65b89DF0B4894ABE91E6D02FE579834F8F",
      },
    );
  });

  await test("precomputes L1 LinethRollup from the latest deployer nonce", () => {
    assert.equal(
      computeBootPrecomputedAddresses({
        l1DeployerAddress: "0xf39fd6e51aad88f6f4ce6ab8827279cfffb92266",
