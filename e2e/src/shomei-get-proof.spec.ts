...
describe("Shomei Linea get proof test suite", () => {
  const linethRollupV8 = context.l1Contracts.linethRollup(context.l1PublicClient());
  ...
  it.concurrent(
    ...
    async () => {
      let targetL2BlockNumber = await awaitUntil(
        async () => {
          try {
            return await linethRollupV8.read.currentL2BlockNumber({ blockTag: "latest" });
          } catch (err) {
      ...
      const getProofResponse = await awaitUntil(
        async () => {
          ...
          if (!getProofResponse) {
            ...
            const latestFinalizedL2BlockNumber = await linethRollupV8.read.currentL2BlockNumber({
              blockTag: "latest",
            });
