...
fun createAppClients(
  ...
  val ethLogsSearcher =
    run {
      val log = LogManager.getLogger("lineth.plugin.staterecovery.clients.l1.logs-searcher")
  ...
  val lineaContractClient =
    Web3JLinethRollupSmartContractClientReadOnly(
      ...
      createWeb3jHttpClient(
        rpcUrl = l1RpcEndpoint.toString(),
        log = LogManager.getLogger("lineth.plugin.staterecovery.clients.l1.smart-contract"),
      ),
  ...
  val blobScanClient =
    BlobScanClient.create(
      ...
      logger = LogManager.getLogger("lineth.plugin.staterecovery.clients.l1.blob-scan"),
  ...
  val stateManagerClient: StateManagerClientV1 =
    StateManagerV1JsonRpcClient.create(
      ...
      logger = LogManager.getLogger("lineth.plugin.staterecovery.clients.state-manager"),
    )
  val transactionDetailsClient: TransactionDetailsClient =
    VertxTransactionDetailsClient.create(
      ...
      logger = LogManager.getLogger("lineth.plugin.staterecovery.clients.l1.transaction-details"),
    )
  return AppClients(
